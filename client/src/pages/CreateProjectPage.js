...
function CreateProjectPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post(`${API_BASE_URL}/projects`, { projectId, owner: user.username });
  ...
  return (
    <CenteredCard title="Create Project">
      <form onSubmit={handleSubmit} style={formStyle}>
        <input type="text" placeholder="Enter new project ID"
          value={projectId} onChange={(e) => setProjectId(e.target.value)} required />
        <button type="submit" style={btnStyle('#28a745')}>Create</button>
      </form>
      {message && <p>{message}</p>}
