...
function HardwareSet1Page() {
  ...
  const handleCheckout = async (qty) => {
    try {
      const res = await axios.post(`${API_BASE_URL}/hardware/hwset1/checkout`, { userId: user.username, projectId, qty });
  ...
  const handleCheckin = async (qty) => {
    try {
      const res = await axios.post(`${API_BASE_URL}/hardware/hwset1/checkin`, { userId: user.username, projectId, qty });
  ...
  return (
    <CenteredCard title="Hardware Set 1">
      <button onClick={() => handleCheckout(1)} style={btnStyle('#28a745')}>Checkout 1 Unit</button>
      <button onClick={() => handleCheckin(1)} style={btnStyle('#ffc107')}>Checkin 1 Unit</button>
...
export default HardwareSet1Page;
